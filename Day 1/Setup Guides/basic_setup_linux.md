...
## Prerequisites
These steps are written for **Ubuntu**, with links for other operating systems. 
1. **Update**: For Ubuntu, press `Ctrl + Alt + T` and run 
    ```bash
    sudo apt update && sudo apt upgrade
    ```
2. **Git**: For other operating systems, see [https://git-scm.com/downloads](https://git-scm.com/downloads)
   ```bash
    sudo apt install git
    git config --global user.name "Your Full Name" 
    git config --global user.email "your.email@example.com"
    ```
3. **Curl**: Curl should already be installed on Ubuntu. If not, install it (see [https://curl.se](https://curl.se) for reference):
    ```bash
    sudo apt install curl
    ```
4. **VS Code**: **SKIP this step IF** you are on Windows using **WSL**. For other operating systems, see [code.visualstudio.com/docs/setup/linux](https://code.visualstudio.com/docs/setup/linux)
    ```bash
...
## Install Elan
In the Lean ecosystem, `elan` is the official version manager for Lean toolchains (similar to `rustup` for Rust or `nvm` for Node.js). For different operating systems, the installation guide is available here: [https://lean-lang.org/install/manual/](https://lean-lang.org/install/manual/).
...
2. **Refresh** your shell: This sources the Elan environment script, which adds `~/.elan/bin` to your `PATH`.
   ```bash
    source ~/.elan/env
    ```
...
1. Create a dedicated folder for your project and let Lake handle the setup:
    ...
    ```
    This will set up the project structure and configure `mathlib` as a dependency. 
...
2. After creating the project, run the following commands:
    ```bash
    cd my_project
    lake update
    lake exe cache get
